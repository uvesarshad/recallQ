...
import { importCsvExport } from "@/lib/import-export-db";
...
export async function POST(req: Request) {
  ...
  try {
    const result = await importCsvExport({
      userId: user.id,
    ...
    return apiError(error instanceof Error ? error.message : "CSV import failed", 500);
  }
}
