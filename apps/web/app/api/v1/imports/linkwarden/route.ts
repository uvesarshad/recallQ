...
import { importLinkwardenExport } from "@/lib/import-export-db";
...
export async function POST(req: Request) {
  ...
  try {
    const result = await importLinkwardenExport({
      userId: user.id,
    ...
    return apiError(error instanceof Error ? error.message : "Linkwarden import failed", 500);
  }
}
