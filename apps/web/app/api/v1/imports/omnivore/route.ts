...
import { importOmnivoreExport } from "@/lib/import-export-db";
...
export async function POST(req: Request) {
  ...
  try {
    const result = await importOmnivoreExport({
      userId: user.id,
    ...
    return apiError(error instanceof Error ? error.message : "Omnivore import failed", 500);
  }
}
